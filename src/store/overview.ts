...
import { getCategorySpends, getSpendAnalytics } from "@/actions/overview";
...
export const useOverviewStore = create<OverviewStore>((set) => ({
  ...
  fetchOverviewData: async () => {
    try {
      set({ isLoading: true });
      const res = await Promise.all([getSpendAnalytics(), getCategorySpends()]);
      const [spendAnalytics, categorySpends] = res;
      set({
        data: {
          spendAnalytics: spendAnalytics.data as any,
          categorySpends: categorySpends.data as any,
        },
        isLoading: false,
      });
    } catch (error) {
